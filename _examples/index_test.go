...
func TestNewApp(t *testing.T) {
	...
	// 测试获取单个
	getSingle := e.GET(prefix + "/" + testModel + "/1").Expect().Status(httptest.StatusOK)
	getSingle.JSON().Object().ContainsKey("name")

	// 测试修改
	editMap := map[string]interface{}{"name": "修改后"}
	editData := e.PUT(prefix + "/" + testModel + "/1").WithForm(editMap).Expect().Status(httptest.StatusOK)
	editData.JSON().Object().Equal(editMap)

	// 测试删除
	deleteData := e.DELETE(prefix + "/" + testModel + "/1").Expect().Status(httptest.StatusOK)
